...
interact in a variety of ways with a specific window.
...
There are two fundamental dialog types: modal and modeless.
...
With combo boxes, a user may select among a list of options or type
in a new item.
...
@section Cursors
Cursors are the visual elements which indicate where a user's input focus
...
new brush will then have the same colors that the user is used
...
include solid, dashed, dotted, etc.

@subsection Stock Pens
Stock pens allow a programmer to select among a few, common, Windows
defined pens.
...
your C compiler package.  The resource editor included with
Microsoft Visual Studio can be used to create and edit standard
@code{.rc} files.  Any resource editor that produces standard Windows
resource files will work with WDS.
...
you decide to use some routine in your common DLL which is used by the
...
@section Windows API
WDS targets the Windows API (64-bit) and builds as a native 64-bit
application on Windows using MSVC.  WDS is also portable to Linux and
macOS via the Wine library, where it likewise builds and runs as a
64-bit application using GCC/winegcc.
